...
object DataValidation {
  ...
  val anInvalidValue: Validated[String, Int] =
    Validated.invalid("Something went wrong")
  ...
  object FormValidation {
    ...
    def getValue(
      ...
      Validated.fromOption(
        form.get(fieldName),
        List(s"The field $fieldName must be specified.")
      )
    ...
    def nonBlank(value: String, fieldName: String): FormValidation[String] =
      Validated.cond(
        value.length > 0,
        value,
        List(s"The field $fieldName must not be blank.")
      )
    ...
    def passwordCheck(password: String): FormValidation[String] =
      Validated.cond(
        password.length >= 10,
        password,
        List("Passwords must be at least 10 characters long.")
      )
